...
from src.data.text_data import Tokenizer
...
def translate_sentence(model, sentence, src_tokenizer, trg_tokenizer, max_length=100, device='cpu'):
    ...
    for _ in range(max_length):
        ...
        with torch.no_grad():
            output = model.decode(memory, src_mask, trg_tensor, trg_mask)
...
def main():
    ...
    parser.add_argument('--direction', type=str, choices=['en2sn', 'sn2en'], default='sn2en',
                       help='Translation direction: en2sn (English to Shona) or sn2en (Shona to English)')
    parser.add_argument('--src_file', type=str, default='data/Train/Flores-200/Shona/shona_100K.txt')
    parser.add_argument('--trg_file', type=str, default='data/Train/Flores-200/English/english_100K.txt')
    ...
    if args.direction == 'sn2en':
        # Shona to English (as trained)
        src_file = args.src_file
        trg_file = args.trg_file
    else:
        # English to Shona (reverse - requires model trained in that direction)
        # Assuming we trained Shona->English for now based on report ("Shona-English Translation")
