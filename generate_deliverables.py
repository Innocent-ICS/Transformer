...
from ml.data.afrispeech_loader import AfriSpeechShona
import jiwer
import numpy as np
...
def generate_pdf_report(wer, output_path="report.pdf"):
    pdf = FPDF()
    pdf.add_page()
...
def main(model_dir):
    ...
    # Files to write to
    txt_file = open("ground_truth.txt", "w", encoding="utf-8")
    tsv_file = open("transcriptions.tsv", "w", encoding="utf-8")
